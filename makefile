...
CXXFLAGS := `pkg-config --cflags OGRE --cflags CEGUI-OGRE` -I$(DIRHEA) -Wall -std=c++11 `pkg-config	\
--cflags OgreBullet` `pkg-config --cflags bullet`


# Flags del linker ---------------------------------------------------
LDFLAGS := `pkg-config --libs-only-L OGRE --libs-only-L CEGUI-OGRE` `pkg-config --libs-only-l  	\
OgreBullet` `pkg-config --libs-only-l bullet` 
LDLIBS := `pkg-config --libs-only-l OGRE --libs-only-L CEGUI-OGRE`  `pkg-config --libs-only-l	\
OgreBullet` `pkg-config --libs-only-l bullet` -lOIS -lstdc++ -lboost_system -lConvexDecomposition -L/usr/local/lib -Wl,-rpath=/usr/local/lib -lSDL -lSDL_mixer
